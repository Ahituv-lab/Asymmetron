...
def table_bins_gen(Score_names,Ratio_Bins,Ratio_Convergent_Divergent_Bins,output_table):
    datafile = open(output_table, "w")
    ...
    for i in range(len(Score_names)):
        datafile.write(str(round(Score_names[i][0],0))+"-"+str(round(Score_names[i][1],0))+'\t'+str(Ratio_Bins[i])+'\t'+str(Ratio_Convergent_Divergent_Bins[i])+'\n')
    datafile.close()
    return
