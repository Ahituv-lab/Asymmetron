...
def fun1(args):
    ...
    for index, path in enumerate(paths):
        name = names[index]
        DataL_significant, consecutiveL, occsL, consecutive_controlL, occs_controlL = functions.asymmetries_single(path,
                                                                                                                   patterns,
                                                                                                                   min_distance,
                                                                                                                   max_distance,
                                                                                                                   threshold,
            ...
            if plots:
                ...
                for k in range(1, max_consecutive + 1):
                    ...
                    if k not in consecutive_control_total:
                        consecutive_control_total = consecutive_control_total[:k - 1] + [k] + consecutive_control_total[
                                                                                              k - 1:]
                        times_found_control_total = times_found_control_total[:k - 1] + [0] + times_found_control_total[
                                                                                              k - 1:]

                visualizations.barplot_single_gen(times_found_total, consecutive_total, "Occurrences",
                ...
                visualizations.barplot_pair_lists_gen(consecutive_total, times_found_control_total, times_found_total,
                                                      "Expected", "Observed", "Consecutive occurrences", '',
                                                      wf.output_path("consecutive_patterns", "png",
                                                                     os.path.basename(path) + "_with_controls",
                                                                     str(patterns[i])))
        ...
        if bins > 1:
            ...
            for pat in patterns:
                consecutiveLL_bin = []
                occsLL_bin = []
                consecutive_controlLL_bin = []
                occs_controlLL_bin = []
                for min_bin, max_bin in Bins:
                    DataL_significant_bin, consecutiveL_bin, occsL_bin, consecutive_controlL_bin, occs_controlL_bin = functions.asymmetries_single(
                        path, [pat], min_bin, max_bin, threshold)
                ...
                consecutiveLL_binT = np.array(consecutiveLL_bin).T.tolist()
                occsLL_binT = np.array(occsLL_bin).T.tolist()
                consecutive_controlLL_binT = np.array(consecutive_controlLL_bin).T.tolist()
                occs_controlLL_binT = np.array(occs_controlLL_bin).T.tolist()

                functions.table_consecutive(consecutiveLL_bin,
                                            ["Bin:" + str(bin_range[0]) + "-" + str(bin_range[1]) for bin_range in
                                             Bins],
                                            wf.output_path("consecutive_patterns", "txt", os.path.basename(path),
                                                           "_Consecutive_Patterns_bins", str(pat)))

                if plots:
                    # Plot barplot of consecutive in each bin  
                    visualizations.heatmap_gen(consecutiveLL_bin, consecutive_controlLL_bin, Bins,
                                               wf.output_path("consecutive_patterns", "png", 'distribution_distances',
                                                              pat))
...
def consecutive_patterns_parser():
    ...
    parser.add_argument("-b", "--bins",
                        ...
                        type=int, default=1)  # Needs rephrasing
