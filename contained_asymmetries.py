...
def fun2(args):
    ...
    for i in range(len(motif_region_pairs)):
        ...
        if plots:
            ...
            visualizations.barplot_gen(same_strand, opposite_strand, "Same", "Opposite",
                                       wf.output_path("contained_asymmetries", "png", "same_opposite_orientation",
            ...
            visualizations.barplot_gen(p_m, m_p, "Convergent", "Divergent",
                                       wf.output_path("contained_asymmetries", "png",
                                                      "convergent_divergent_orientation", names_pairs[i][0],
                                                      names_pairs[i][1]))

        if score:
            Ratio_Bins, Ratio_Convergent_Divergent_Bins, Score_names, Binom_Test_Same_Opposite, Binom_Test_Same_Opposite_Bonferoni, Binom_Test_Convergent_Divergent, Binom_Test_Convergent_Divergent_Bonferoni = functions.separate_on_score(
                motif_region_pairs[i][1], motif_region_pairs[i][0], bins_score, number_of_files, expected_asym,
                expected_asym_conv_div)
            functions.table_bins_gen(Score_names, Ratio_Bins, Ratio_Convergent_Divergent_Bins, Binom_Test_Same_Opposite,
                                     Binom_Test_Same_Opposite_Bonferoni, Binom_Test_Convergent_Divergent,
                                     Binom_Test_Convergent_Divergent_Bonferoni,
                                     wf.output_path("contained_asymmetries", "txt", "Table_Strand_Asymmetries_Scores",
                                                    names_pairs[i][0], names_pairs[i][1]))

            visualizations.barplot_single_gen(Ratio_Bins,
                                              [(int(round(score_name[0], 0)), int(round(score_name[1], 0))) for
                                               score_name in Score_names], "Strand Asymmetry", "Score",
                                              wf.output_path("contained_asymmetries", "png",
                                                             "same_opposite_orientation_separated_score",
                                                             names_pairs[i][0], names_pairs[i][1]))
            visualizations.barplot_single_gen(Ratio_Convergent_Divergent_Bins,
                                              [(int(round(score_name[0], 0)), int(round(score_name[1], 0))) for
                                               score_name in Score_names], "Strand Asymmetry", "Score",
                                              wf.output_path("contained_asymmetries", "png",
                                                             "convergent_divergent_orientation_separated_score",
                                                             names_pairs[i][0], names_pairs[i][1]))
    ...
    functions.table_gen(names_pairs, p_pL, m_mL, p_mL, m_pL, p_val_same_oppositeL, p_val_same_opposite_BonferoniL,
                        Ratio_same_oppositeL, p_val_conv_divergL, p_val_conv_diver_BonferoniL, Ratio_conv_divergL,
                        wf.output_path("contained_asymmetries", "txt", "all_assymetries"))
